...
//   }
//   auto t =
//   leza::compression::huffman::classic::core::getFrequencyArray(data); for
...
TEST(HuffmanCompressorTest, deserializeTest) {
  ...
  std::println("readCount = {}", deSerializeResult.readCount);

  auto decodeResult = decode(outputData);
  for (auto d : decodeResult.data) {
    std::print("{} ", d);
  }
  std::println();
